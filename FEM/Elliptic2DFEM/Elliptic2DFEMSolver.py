class gridClass:
    ...
    def __init__(self, minv, maxv, N):
        ...
        self.gridX = np.array([-1, 1, 1, -1])
        self.gridY = np.array([-1, -1, 1, 1])
        self.EToV = np.matrix([[1, 2, 4],[2, 3, 4]])
    ...
    def plot_grid(self):
        ...
        print len(np.squeeze(np.asarray(self.EToV[:,0])))
        numTri = len(self.EToV[:,0])
        X = np.zeros((4, numTri))
        Y = np.zeros((4, numTri))

        for k in range(0, numTri):
            print k
            X[0,k] = self.gridX[self.EToV[k,0] - 1]
            X[1,k] = self.gridX[self.EToV[k,1] - 1]
            X[2,k] = self.gridX[self.EToV[k,2] - 1]
            X[3,k] = X[0,k]

            Y[0,k] = self.gridY[self.EToV[k,0] - 1]
            Y[1,k] = self.gridY[self.EToV[k,1] - 1]
            Y[2,k] = self.gridY[self.EToV[k,2] - 1]
            Y[3,k] = Y[0,k]
            plt.plot(X[:,k], Y[:,k])
